...
%   which of the available data to plot.
    ...
    root_names = fieldnames(summary_data);
    
    % Create and populate structure with all child field names
    child_names = struct;
    for i = 1:length(root_names)
        tmp = fieldnames(getfield(summary_data, root_names{i}));
        child_names.(root_names{i}) = tmp;
    end            
    
    % Create figure as root gui element
    fig_height = 800; fig_width = 900;
    fh = figure('Visible','on','Position',[360,500,fig_width,fig_height]);
    ...
    for i = 1:length(root_names)
        parent_value = root_names{i};
        child_struct = getfield(child_names, parent_value);
    ...
    button_close = uicontrol(fh, 'Style', 'pushbutton', ...
    ...
    function checkbox_clicked(source, eventdata, handles)
